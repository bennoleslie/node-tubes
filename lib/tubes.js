...
exports.StaticFile = require('./staticfile.js').StaticFile
exports.SlowDown = require('./slowdown.js').SlowDown

var tubes = exports /* alias the current module */

function R(filename) { return __dirname + "/" + filename }

exports.duration = { minute: 60, hour: 3600, day: 86400, year: 604800, month: 2592000, year: 22896000}
...
function Server(name, service, logger, options) {
    ...
    if (options.secure === true) {
        ...
        this.s = https.createServer(https_options)
    } else {
...
Server.prototype.listen = function listen() {
    if (this.fd) {
        this.logger.debug("Listening on socket:", this.fd)
...
Transaction.prototype.pushResHeaders = function pushResHeaders(headers) {
    for (var key in headers) {
        this.resHeaders[key] = headers[key]
    }
}
...
Transaction.prototype.resumeRequest = function resumeRequest() {
    /* Resume could cause pause to be called! So we have to bail */
    if (!this.paused) { return }
    this.paused = false
    for (var i = this.requestChain.length - 1; i >= 0; i--) {
...
Transaction.prototype.httpError = function httpError(code, message) {
    this.status = code
    this.message = message !== undefined ? message : http.STATUS_CODES[code]
...
exports.blankFavicon = new tubes.StaticFile(R('../data/favicon.ico'), {'content-type': 'image/x-icon', 'max-age' : tubes.duration.day})
