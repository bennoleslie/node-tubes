...
var mt = new mimetypes.Mimetypes()
...
AssetManager.prototype.process = function process(trans) {
    ...
    if (!res) {
        ...
        var filename = res.handler.getFile.apply(res.handler, args)
        if (this.devel) {
            /* regenerate */
		...
		var guessType = mt.guessType(filename)
		if (guessType) {
		    headers['content-type'] = guessType
		}

		trans.pushResHeaders(headers)
                trans.status = 200
...
StaticDir.prototype.regenerate = function regenerate(callback, file) {
